    ...
    <aside :class="sidebarClass">
      ...
      <div class="sidebar-foot">
        ...
        <button
          ...
          :class="{ compact: sidebarCollapsed }"
          type="button"
          :title="sidebarCollapsed ? '灞曞紑' : '鏀惰捣'"
          @click="toggleSidebar"
        >
          <span class="tool-icon collapse-icon" :class="{ collapsed: sidebarCollapsed }">禄</span>
          <span v-if="!sidebarCollapsed">鏀惰捣</span>
        </button>
      </div>
    </aside>
...
async function handleAdminAvatarChange(event: Event) {
  ...
  try {
    ...
    avatarNotice.value = '头像更新成功'
  } catch (error) {
    avatarNotice.value = error instanceof Error ? error.message : '头像更新失败'
  } finally {
