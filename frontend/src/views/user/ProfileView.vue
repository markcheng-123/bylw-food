<template>
  <section class="profile-page">
    ...
    <main class="profile-main">
      ...
      <section v-if="activeMenu === 'profile'" class="content-grid">
        ...
        <article class="content-card security-card">
          <h3>账号安全</h3>
          <p>建议定期更新密码，保护账号安全。</p>
          <button type="button" class="btn outline" @click="openPasswordDialog">修改密码</button>
        </article>
      </section>
    ...
    <div v-if="passwordDialogVisible" class="dialog-mask" @click="closePasswordDialog">
      <article class="dialog-card password-dialog" @click.stop>
        <header>
          <h4>修改密码</h4>
          <button type="button" class="btn ghost" :disabled="passwordSubmitting" @click="closePasswordDialog">关闭</button>
        </header>
        <div class="form-grid">
          <label class="field full-row">
            <span>当前密码</span>
            <input v-model="passwordForm.currentPassword" :type="passwordVisible ? 'text' : 'password'" autocomplete="current-password" placeholder="请输入当前密码" />
          </label>
          <label class="field full-row">
            <span>新密码</span>
            <input v-model="passwordForm.newPassword" :type="passwordVisible ? 'text' : 'password'" autocomplete="new-password" placeholder="请输入新密码（6-20位）" />
          </label>
          <label class="field full-row">
            <span>确认新密码</span>
            <input v-model="passwordForm.confirmPassword" :type="passwordVisible ? 'text' : 'password'" autocomplete="new-password" placeholder="请再次输入新密码" />
          </label>
        </div>
        ...
        <footer class="dialog-foot password-foot">
          <button type="button" class="btn ghost" :disabled="passwordSubmitting" @click="closePasswordDialog">取消</button>
          <button type="button" class="btn primary" :disabled="passwordSubmitting" @click="handleChangePassword">{{ passwordSubmitting ? '提交中...' : '确认修改' }}</button>
        </footer>
      </article>
    </div>
  </section>
...
async function handleChangePassword() {
  ...
  if (!passwordForm.currentPassword || !passwordForm.newPassword || !passwordForm.confirmPassword) {
    passwordNotice.value = '请完整填写密码信息'
    return
  }
  if (passwordForm.newPassword.length < 6 || passwordForm.newPassword.length > 20) {
    passwordNotice.value = '新密码长度需在 6-20 位之间'
    return
  }
  ...
  try {
    ...
    passwordNotice.value = error instanceof Error ? error.message : '密码修改失败'
  } finally {
...
.password-dialog { width: min(520px, 96vw); }
...
.password-foot { justify-content: flex-end; gap: 10px; }
.password-visibility { display: inline-flex; align-items: center; gap: 8px; margin-top: 8px; color: #7d6d5e; font-size: 13px; }
.password-visibility input { width: 14px; height: 14px; }
